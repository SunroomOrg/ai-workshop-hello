...
const PIXEL = PIXEL_FONT_STACK;

/**
 * 16-bit world map of Lan's life, ported from the main site
 * (src/components/MapBackground.tsx) into the video project. Pure SVG —
 * no DOM-time animations, so it renders identically frame-to-frame
 * inside Remotion. ViewBox is 140 × 60.
 */
/**
 * Padded 16:9 viewBox so the entire 140×60 map fits inside a 1920×1080
 * frame without slicing off Vietnam (left) or Austin (right). Original
 * content sits at y=0..60; we pad the sky upward so the new aspect
 * matches the video. Keep this in sync with the overlay SVG in IntroScene.
 */
export const MAP_VIEWBOX_X = -1;
export const MAP_VIEWBOX_Y = -19;
export const MAP_VIEWBOX_W = 142;
export const MAP_VIEWBOX_H = 79.875; // 142 / (16/9)
export const MAP_VIEWBOX = `${MAP_VIEWBOX_X} ${MAP_VIEWBOX_Y} ${MAP_VIEWBOX_W} ${MAP_VIEWBOX_H}`;
...
function UTTower({ baseX, baseY }: { baseX: number; baseY: number }) {
  ...
  return (
    <g>
      ...
      {[15, 13, 11, 9, 7].map((y) => (
        <g key={y}>
          <rect x={baseX + 2.7} y={baseY - y} width={0.4} height={0.8} fill={white} />
          <rect x={baseX + 3.5} y={baseY - y} width={0.4} height={0.8} fill={white} />
          <rect x={baseX + 4.3} y={baseY - y} width={0.4} height={0.8} fill={white} />
          <rect x={baseX + 5.1} y={baseY - y} width={0.4} height={0.8} fill={white} />
        </g>
      ))}
...
function SaturnVRocket({ baseX, baseY }: { baseX: number; baseY: number }) {
  const W = 3.2;
  const x = baseX;
...
function MissionControl({ baseX, baseY }: { baseX: number; baseY: number }) {
  return (
    <g>
...
function NasaMeatball({ cx, cy, r }: { cx: number; cy: number; r: number }) {
  return (
    <g>
...
function AustinHome() {
  return (
    <g>
      ...
      <polygon
        points="118,42 140,42 140,28 134,24 128,22 122,24 118,28"
        fill="#5a8a4a"
      />
      <polygon
        points="118,42 140,42 140,32 134,28 128,26 122,28 118,32"
        fill="#5a8a4a"
      />
...
function Capitol({ baseX, baseY }: { baseX: number; baseY: number }) {
  ...
  return (
    <g>
      ...
      <rect x={baseX + 5.4} y={baseY - 12.4} width={1.2} height={0.4} fill={graniteDk} />
      <rect
        x={baseX + 5.5}
        y={baseY - 13.4}
        width={1}
